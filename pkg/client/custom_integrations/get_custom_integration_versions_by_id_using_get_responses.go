// Code generated by go-swagger; DO NOT EDIT.

package custom_integrations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	models "github.com/vmware/vra-sdk-go/pkg/models"
)

// GetCustomIntegrationVersionsByIDUsingGETReader is a Reader for the GetCustomIntegrationVersionsByIDUsingGET structure.
type GetCustomIntegrationVersionsByIDUsingGETReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetCustomIntegrationVersionsByIDUsingGETReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewGetCustomIntegrationVersionsByIDUsingGETOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 401:
		result := NewGetCustomIntegrationVersionsByIDUsingGETUnauthorized()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	case 403:
		result := NewGetCustomIntegrationVersionsByIDUsingGETForbidden()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	case 404:
		result := NewGetCustomIntegrationVersionsByIDUsingGETNotFound()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	case 500:
		result := NewGetCustomIntegrationVersionsByIDUsingGETInternalServerError()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result

	default:
		return nil, runtime.NewAPIError("unknown error", response, response.Code())
	}
}

// NewGetCustomIntegrationVersionsByIDUsingGETOK creates a GetCustomIntegrationVersionsByIDUsingGETOK with default headers values
func NewGetCustomIntegrationVersionsByIDUsingGETOK() *GetCustomIntegrationVersionsByIDUsingGETOK {
	return &GetCustomIntegrationVersionsByIDUsingGETOK{}
}

/*GetCustomIntegrationVersionsByIDUsingGETOK handles this case with default header values.

'Success' with requested versions for Custom Integration
*/
type GetCustomIntegrationVersionsByIDUsingGETOK struct {
	Payload models.CustomIntegrations
}

func (o *GetCustomIntegrationVersionsByIDUsingGETOK) Error() string {
	return fmt.Sprintf("[GET /codestream/api/custom-integrations/{id}/versions][%d] getCustomIntegrationVersionsByIdUsingGETOK  %+v", 200, o.Payload)
}

func (o *GetCustomIntegrationVersionsByIDUsingGETOK) GetPayload() models.CustomIntegrations {
	return o.Payload
}

func (o *GetCustomIntegrationVersionsByIDUsingGETOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload as interface type
	payload, err := models.UnmarshalCustomIntegrations(response.Body(), consumer)
	if err != nil {
		return err
	}
	o.Payload = payload

	return nil
}

// NewGetCustomIntegrationVersionsByIDUsingGETUnauthorized creates a GetCustomIntegrationVersionsByIDUsingGETUnauthorized with default headers values
func NewGetCustomIntegrationVersionsByIDUsingGETUnauthorized() *GetCustomIntegrationVersionsByIDUsingGETUnauthorized {
	return &GetCustomIntegrationVersionsByIDUsingGETUnauthorized{}
}

/*GetCustomIntegrationVersionsByIDUsingGETUnauthorized handles this case with default header values.

Unauthorized Request
*/
type GetCustomIntegrationVersionsByIDUsingGETUnauthorized struct {
}

func (o *GetCustomIntegrationVersionsByIDUsingGETUnauthorized) Error() string {
	return fmt.Sprintf("[GET /codestream/api/custom-integrations/{id}/versions][%d] getCustomIntegrationVersionsByIdUsingGETUnauthorized ", 401)
}

func (o *GetCustomIntegrationVersionsByIDUsingGETUnauthorized) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewGetCustomIntegrationVersionsByIDUsingGETForbidden creates a GetCustomIntegrationVersionsByIDUsingGETForbidden with default headers values
func NewGetCustomIntegrationVersionsByIDUsingGETForbidden() *GetCustomIntegrationVersionsByIDUsingGETForbidden {
	return &GetCustomIntegrationVersionsByIDUsingGETForbidden{}
}

/*GetCustomIntegrationVersionsByIDUsingGETForbidden handles this case with default header values.

Forbidden
*/
type GetCustomIntegrationVersionsByIDUsingGETForbidden struct {
}

func (o *GetCustomIntegrationVersionsByIDUsingGETForbidden) Error() string {
	return fmt.Sprintf("[GET /codestream/api/custom-integrations/{id}/versions][%d] getCustomIntegrationVersionsByIdUsingGETForbidden ", 403)
}

func (o *GetCustomIntegrationVersionsByIDUsingGETForbidden) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewGetCustomIntegrationVersionsByIDUsingGETNotFound creates a GetCustomIntegrationVersionsByIDUsingGETNotFound with default headers values
func NewGetCustomIntegrationVersionsByIDUsingGETNotFound() *GetCustomIntegrationVersionsByIDUsingGETNotFound {
	return &GetCustomIntegrationVersionsByIDUsingGETNotFound{}
}

/*GetCustomIntegrationVersionsByIDUsingGETNotFound handles this case with default header values.

Not Found
*/
type GetCustomIntegrationVersionsByIDUsingGETNotFound struct {
}

func (o *GetCustomIntegrationVersionsByIDUsingGETNotFound) Error() string {
	return fmt.Sprintf("[GET /codestream/api/custom-integrations/{id}/versions][%d] getCustomIntegrationVersionsByIdUsingGETNotFound ", 404)
}

func (o *GetCustomIntegrationVersionsByIDUsingGETNotFound) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewGetCustomIntegrationVersionsByIDUsingGETInternalServerError creates a GetCustomIntegrationVersionsByIDUsingGETInternalServerError with default headers values
func NewGetCustomIntegrationVersionsByIDUsingGETInternalServerError() *GetCustomIntegrationVersionsByIDUsingGETInternalServerError {
	return &GetCustomIntegrationVersionsByIDUsingGETInternalServerError{}
}

/*GetCustomIntegrationVersionsByIDUsingGETInternalServerError handles this case with default header values.

Server Error
*/
type GetCustomIntegrationVersionsByIDUsingGETInternalServerError struct {
}

func (o *GetCustomIntegrationVersionsByIDUsingGETInternalServerError) Error() string {
	return fmt.Sprintf("[GET /codestream/api/custom-integrations/{id}/versions][%d] getCustomIntegrationVersionsByIdUsingGETInternalServerError ", 500)
}

func (o *GetCustomIntegrationVersionsByIDUsingGETInternalServerError) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}
